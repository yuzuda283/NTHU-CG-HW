...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
	...
	case GeoRotation:
		...
		break;
	case Shine:
		shininess += 5 * (float)yoffset;
		break;
	case Changelightmode:
		if (light_idx % 3 == 0)
		{
			Ld1 = Ld1 + Vector3(1, 1, 1)*(float)yoffset;
		}
		else if (light_idx % 3 == 1)
		{
			Ld2 = Ld2 + Vector3(1, 1, 1)*(float)yoffset;
		}
		else if (light_idx % 3 == 2)
		{
			spotcutoff += -10 * (float)yoffset;
		}
	default:
		return;
		break;
...
GLuint LoadTextureImage(string image_path)
{
	...
	if (data != NULL)
	{
		...
		// Hint: glGenTextures, glBindTexture, glTexImage2D, glGenerateMipmap4
		glGenTextures(1, &tex);
...
int main(int argc, char **argv)
{
    ...
    // create window
	GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "108065466 HW3", NULL, NULL);
    if (window == NULL)
    {
